...
class car_manual_generator():
    ...
    def context_parser(self, docs):
        ...
        parsed_result = {
            'page_contents': [s for s in page_contents],
            'img_urls': [s for s in img_urls],
            'table_img_urls': [s for s in tbl_img_urls],
            'table_csv_urls': [s for s in tbl_csv_urls],
            # 'doc_id': doc_ids
        }
    ...
    def create_context_bag(self, docs, bag_size=4000): 
        ...
        context_bag = {
            ...
            'image_urls': [s for s in image_urls],
            'table_image_urls': [s for s in table_image_urls],
            'table_csv_urls': [s for s in table_csv_urls],
    ...
    async def _agenerate_answer(self, query, session_id, stream_it: AsyncIteratorCallbackHandler):
        ...
        history_image = RedisChatMessageHistory(session_id+'_image', url=REDIS_URL)
        context_root_dir = self.context_path
        ...
        # reduce_answer = await chain_with_history.ainvoke({"question": query}, config=config)
